...
func (r *UserRepo) CancelLeaveRequest(userID string, leaveID int) error {
    // 🌟 เปลี่ยนจากการ DELETE เป็นการ UPDATE status 
    result := r.db.Table("leave_requests").
        Where("id = ? AND user_id = ?", leaveID, userID).
        Update("status", "canceled") // หรือใช้ "cancelled" ตามที่ตกลงกับ Frontend

    // เช็คว่า Database มี Error ไหม
    if result.Error != nil {
        return result.Error
    }

    // เช็คว่ามี record ถูกอัปเดตจริงๆ ไหม (ป้องกันคนส่ง ID มั่ว หรือพยายามยกเลิกใบลาของคนอื่น)
